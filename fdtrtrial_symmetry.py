        ...
        for ix in range(len(qx)):
            ...
            for iy in range(len(qy)):
                ...
                for iz in range(len(qz)):
                    qzi = qz[iz]
                    dT += fdtr2d_gray(wi, qxi, qyi, qzi, delta, R, vx, tau, CC)
                    if qxi != 0:
                        dT += fdtr2d_gray(wi, -qxi, qyi, qzi, delta, R, vx, tau, CC)
                    if qyi != 0:
                        dT += fdtr2d_gray(wi, qxi, -qyi, qzi, delta, R, vx, tau, CC)
                    ...
                    if qyi != 0 and qzi != 0:
                        dT += fdtr2d_gray(wi, qxi, -qyi, -qzi, delta, R, vx, tau, CC)
                    if qxi != 0 and qyi != 0 and qzi != 0:
                        dT += fdtr2d_gray(wi, -qxi, -qyi, -qzi, delta, R, vx, tau, CC)
        deltaT[i] = dT
    return deltaT

# Spatial frequency Fourier transform variables (positive domain only)
qx = np.linspace(1e-8, 1e8, 300)  # Start from 1e-8 to avoid zero
qy = np.linspace(1e-8, 1e8, 300)  # Start from 1e-8 to avoid zero
qz = np.linspace(1e-8, 1e8, 300)  # Start from 1e-8 to avoid zero
